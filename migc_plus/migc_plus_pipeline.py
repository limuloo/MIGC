...
import cv2
import math
from migc_plus.migc_plus_arch import MIGC_plus, RefinedShader
...
class MIGCPlusProcessor(nn.Module):
    ...
    def __call__(
        ...
        is_vanilla_cross = (not_use_migc and ith > RefinedSteps)
        if instance_num == 0:
        ...
        # In this case, we need to use MIGC or RefinedShader, so we copy the hidden_states_cond (instance_num+1) times for QKV
        ...
        if masks is None:
            ...
            guidance_masks = [o[None, ...] for o in masks]
            for bbox in bboxes[0]:
                in_box.append([bbox[0], bbox[2], bbox[1], bbox[3]])
        ...
        if not not_use_migc:
            ...
            hidden_states_cond, fuser_info = self.refined_shader(cond_ca_output,
                                            guidance_masks,
                                            other_info=other_info,
                                            return_fuser_info=True)
...
class StableDiffusionMIGCPlusPipeline(StableDiffusionPipeline):
    def __init__(
            self,
            vae: AutoencoderKL,
    ...
    @staticmethod
    def apply_colored_mask_combined(image_input, mask: np.ndarray, color: str, alpha: float) -> np.ndarray:
        """
        Apply a colored mask to an input that can be either a PIL.Image object or a NumPy array,
        based on a binary mask (0 and 1), and return the result as a NumPy array.

        Parameters:
        - image_input: The original image, either as a PIL.Image object or a NumPy array.
        - mask: A binary (0 and 1) mask, where 1 indicates the pixels to be colored.
        - color: A string indicating the color to apply ('green', 'red', 'blue', 'brown', 'yellow', 'white').
        - alpha: A float indicating the transparency of the mask (0 to 1).

        Returns:
        - The modified image as a NumPy array.
        """
        # Check if the input is a PIL.Image object and convert it to a NumPy array if so
        ...
        mask_3_channel = np.dstack([mask] * 3)

        # Get the color in RGB format
        color_rgb = np.array(color_dict[color], dtype=np.uint8)
        ...
        blended_image = np.where(mask_3_channel == 1, ((1 - alpha) * image + alpha * colored_mask).astype(np.uint8), image)
